...
locals {
  ...
  api_env_vars_mysql = {
    ...
    todo_user = "foo"
    todo_pass = var.database_type == "mysql"
...
# ---------------------------------------------------------------------------------
# Resource: Generate a small random ID to suffix the DB instance name
  ...
  template {
    spec {
      ...
      containers {
        ...
        # expose port 80 to match your container’s listening port
